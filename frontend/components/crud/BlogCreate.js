...
const publishBlog = (e) => {
  ...
  //    console.log('ready to publish blog')
   createBlog(formData,token).then(data => {
     ...
     setValues({...values,title:'',error:'',success:`A new blog ${data.title} successfully created`})
     setBody('');
     setCategories([]);
     setTags([]);
    }
  ...
  <input  type="text" className="form-control" value={title} onChange={handleChange('title')} />
    ...
    return (
        ...
        <hr></hr>
           ...
           <small className="text-muted"> maxium size: 1MB </small>
            <label className="btn btn-outline-info"> Upload featured Image 
            <input onChange={handleChange('photo')} type="file" accept="image/*" hidden /></label>
               </div>
           <h5> Categories </h5>
          <hr />
